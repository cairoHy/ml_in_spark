...
def splitDatasetToDirectory(chosenFile):
    """将NetFlix数据集目录下的小文件随机选择1%复制到另外一个文件夹"""
    path = sys.argv[1]
...
def splitDatasetToHDFS(chosenFile):
    """将NetFlix数据集目录下的小文件随机选择1%上传至HDFS中以测试使用"""
    path = sys.argv[1]
    for file in chosenFile:
    ...
    while trigger:
        try:
            ...
            print("1 -> 抽取 “" + sys.argv[1] + 
                "” 目录下1%的文件并复制到“../little/training_set/”目录下\n")
            print("2 -> 抽取 “" + sys.argv[1] + 
                "” 目录下1%的文件并上传到HDFS中“zhy/data/NetFlix/little/training_set/”目录下\n")
